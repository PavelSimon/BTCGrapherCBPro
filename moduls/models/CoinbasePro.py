...
class AuthAPI(AuthAPIBase):
    ...
    def getOrders(self, market='', action='', status='all'):
        ...
        # if status != "open" or status != "done":
        #print(f"resp:\n{resp[resp['status'] != 'done']}")

        if len(resp) > 0:
            if status == 'open':
                df = resp.copy()[['id', 'created_at', 'product_id',
                                  'side', 'type', 'size', 'price', 'status']]
                df['value'] = float(df['price']) * float(df['size'])
                # print(f"df z open:\n {df}")
            else:
                df = resp.copy()[['id', 'created_at', 'product_id', 'side', 'type',
                                  'filled_size', 'executed_value', 'fill_fees', 'status']]
                # print(
                #     f"df:\n {df[df['status']=='open']}\n resp:{resp.columns.values}\n df:{df.columns.values}")
        else:
            return pd.DataFrame()
        ...
        # rename the columns
        if status == 'open':
            df.columns = ['id', 'created_at', 'market', 'action',
                          'type', 'size', 'price', 'status', 'value']
            df = df[['id', 'created_at', 'market', 'action',
                     'type', 'size', 'value', 'status', 'price']]
        else:
            df.columns = ['id', 'created_at', 'market', 'action',
                          'type', 'size', 'value', 'fees', 'status', 'price']
            df = df[['id', 'created_at', 'market', 'action', 'type',
    ...
    def getOrders_raw(self, market='', action='', status='all'):
        """Retrieves your list of orders with optional filtering"""
        ...
        # GET /orders?status
        resp = self.authAPI('GET', 'orders?status=all')
        # if status != "open" or status != "done":
        #print(f"resp:\n{resp[resp['status'] != 'done']}")
    ...
    def marketBuy(self, market='', quote_quantity=0):
        ...
        model = AuthAPI(self.api_key, self.api_secret,
                        self.api_pass, self.api_url)
    ...
    def limitBuy(self, market='', base_quantity=0, futurePrice=0):
        if not self._isMarketValid(market):
            raise ValueError('Coinbase Pro market is invalid.')
    ...
    def authAPI(self, method, uri, payload=''):
        ...
        try:
            if method == 'DELETE':
                ...
                resp = requests.post(self.api_url + uri,
